"""
评估 No-CoT 模型在 Spider 数据集上的 Text-to-SQL 性能。
模型直接输出 SQL，不经过 CoT 推理。
...
def parse_args():
    ...
    parser.add_argument("--test_file", type=str,
                        default=str(PROJECT_DIR / "sft_data" / "test_eval_nocot.json"))
    parser.add_argument("--db_dir", type=str, default=None)
    parser.add_argument("--output_file", type=str, default=None)
    parser.add_argument("--max_new_tokens", type=int, default=512)
    parser.add_argument("--base_model", type=str, default=None,
                        help="Base model path (auto-detected from adapter_config if not set)")
...
def main():
    ...
    for i, item in enumerate(test_data):
        ...
        if (i + 1) % 50 == 0:
            interim_file = args.output_file or str(PROJECT_DIR / "outcome" / "eval_results_nocot.json")
    ...
    output_file = args.output_file or str(PROJECT_DIR / "outcome" / "eval_results_nocot.json")
